...
# Example:
# python Exception_Classification.py
...
def main(args):
    # read dataframe
    raw_data = pd.read_csv(args.raw_data_path,index_col=0)
    exception_data = pd.read_csv(args.exception_data_path, index_col=0) # change the path
    print("Start wrangling training data...")
    train = training_wrangling(raw_data)
    print("Training data wrangling complete")
    print("Start wrangling test data...")
    test = prediction_wrangling(exception_data)
    print("Test data wrangling complete")
    print("Start exception prediction...")
    result = exception_prediction(train,test)
    print("Prediction complete")
    result.to_csv(os.path.join(args.output_data_path,"classification_result.csv"))

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--raw_data_path", type=str,
                        help="raw data file path")
    parser.add_argument("--exception_data_path", type=str,
                        help="exception data file path")
